...
function MakeNotifi(Configs)
  local Title = Configs.Title or "ZeesVN"
...
-- Hàm kiểm tra Key qua API mới (Sử dụng game:HttpGet và HttpService)
local function CheckKeyAPI(key)
    -- URL kiểm tra key của bạn
    local url = string.format("https://zees.rf.gd/robloxhub/check.php?key=%s", key)
    local success, response = pcall(function()
        -- Sử dụng game:HttpGet cho môi trường Roblox
...
function MakeWindow(Configs)
  local title = Configs.Hub.Title or "ZeesVN"
  local Anim_Title = Configs.Hub.Animation or "by : @ZeesVN"
  ...
  local KeyKey = Configs.Key.Keys or {} 
  local KeyLink = Configs.Key.KeyLink or ""
  local KeyNotifications = Configs.Key.Notifi.Notifications or true
  local KeyNotSuccess = Configs.Key.Notifi.Incorrectkey or "Key invalid"
  local KeySuccess = Configs.Key.Notifi.CorrectKey or "Loading script..."
  ...
  if KeySystem then
    ...
    ConfirmButton.MouseButton1Click:Connect(function()
      ...
      -- *** LOGIC KIỂM TRA KEY MỚI (Sử dụng API) ***
      local verified, message = CheckKeyAPI(keyInput)
      KeyVerify = verified
      ...
      if KeyNotifications then
        if KeyVerify then
          ...
          MakeNotifi({
            Title = KeyTitle,
            Text = message, -- Sử dụng thông báo chi tiết từ API
            Time = 5
          })
        end
      end
